    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            .padding(.all, 20.0)
                ...
                .rotation3DEffect(.degrees(proxy.frame(in: .global).minX / -10), axis: (x: -40, y: 1, z: 0))
                .padding(.horizontal, 20)
            /*.shadow(color: Color("Shadow").opacity(0.3), radius: 10, x: 0, y: 10)*/
                ...
                .overlay(
                    ZStack {
                        ...
                        .padding(.top)
                        .offset(y: -40)
                        .offset(x: proxy.frame(in: .global).minX-80 / 2)
                )
        }
    }
}
